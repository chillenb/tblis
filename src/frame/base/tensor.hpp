...
#include "external/marray/marray/marray_view.hpp"
                           ...
                           const stride_vector& stride_B)
{
    ...
    auto dims_A = internal::sort_by_stride(stride_A);
    ...
    auto dims_B = internal::sort_by_stride(stride_B);
...
void fold(len_vector& lengths, label_vector& idx, stride_vector& stride0, Strides&... _strides)
{
    ...
    auto inds = internal::sort_by_stride(stride0);
    ...
    for (auto i : range(N))
        TBLIS_ASSERT(internal::are_compatible(oldlengths, oldstrides[i],
                                            lengths, *strides[i]));
}
                     ...
                     label_vector& idx_out)
{
    ...
    stl_ext::sort(inds, internal::sort_by_idx(idx_in));

...
#include "external/marray/marray/indexed/indexed_marray_view.hpp"
                   ...
                   const indexed_marray_view<char>& A)
{
    ...
    auto stride_A2 = MArray::detail::strides(len_A, MArray::COLUMN_MAJOR);
    ...
    for (auto i : range(A.num_indices()))
    {
        ...
        for (auto j : range(dense_ndim_A,ndim_A))
            data_A2 += idx_A[j-dense_ndim_A]*stride_A2[j]*ts;

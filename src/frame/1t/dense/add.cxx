         ...
         const scalar&  beta, bool conj_B,       char* B)
{
    scalar sum(0.0, type);
    len_type idx;
    reduce(type, comm, cntx, REDUCE_SUM, len_A, A, stride_A, sum.raw(), idx);
         ...
         const stride_vector& stride_B)
{
    ...
    shift(type, comm, cntx, len_B, alpha_A,
    ...
    [&](len_type n_min, len_type n_max)
    {
        ...
        for (len_type i = n_min;i < n_max;i++)
        {
            ...
            while (iter_A.next(A1))
                reduce_ukr(REDUCE_SUM, n0, A1, stride_A0, sum_A.raw(), dummy);
    ...
    [&](len_type n_min, len_type n_max)
    {
        ...
        for (len_type i = n_min;i < n_max;i++)
        {
            ...
            while (iter_B.next(B1))
                shift_ukr(n0, &alpha_A, &beta, conj_B, B1, stride_B0);
        }
    });
}
         ...
         const stride_vector& stride_B_AB)
{
    ...
    const len_type MR = bli_cntx_get_blksz_def_dt((num_t)type, (bszid_t)MRT_BSZ, cntx);
    const len_type NR = bli_cntx_get_blksz_def_dt((num_t)type, (bszid_t)NRT_BSZ, cntx);
         ...
         const stride_vector& stride_B_AB_)
{
    ...
    if (n_A == 0)
    {
        scale(type, comm, cntx, len_B_, beta, conj_B, B, stride_B_);
        return;
    }

    auto perm_A = internal::sort_by_stride(stride_A_);
    auto perm_B = internal::sort_by_stride(stride_B_);
    auto perm_AB = internal::sort_by_stride(stride_B_AB_, stride_A_AB_);
    ...
    if (n_AB == 1)
    {
        if (n_A > 1)
        {
            add(type, comm, cntx, len_A,
                alpha, conj_A, A, stride_A,
                 beta, conj_B, B);
        }
        else if (n_B > 1)
        {
            add(type, comm, cntx, len_B,
                alpha, conj_A, A,
                 beta, conj_B, B, stride_B);
        }
        else if (comm.master())
    ...
    else
    {
        if (n_A > 1)
        {
            add(type, comm, cntx, len_A, len_AB,
                alpha, conj_A, A, stride_A, stride_A_AB,
                 beta, conj_B, B, stride_B_AB);
        }
        else if (n_B > 1)
        {
            add(type, comm, cntx, len_B, len_AB,
